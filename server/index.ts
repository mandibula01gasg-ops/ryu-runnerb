...
app.get("/api/detect-location", async (req, res) => {
  try {
    ...
    console.log("🌍 Detectando localização via ip-api.com para IP:", clientIp);
    
    // Usar o IP do cliente na requisição (aceitar IPv4 e IPv6 puro, mas não "unknown")
    const apiUrl = clientIp && clientIp !== 'unknown' && !clientIp.startsWith('::1') && clientIp !== '127.0.0.1'
      ? `http://ip-api.com/json/${clientIp}?lang=pt-BR`
    ...
    console.log("📍 Dados recebidos da API para", clientIp, ":", data);
    
    const city = data.city || "Sua cidade";
    const region = data.regionName || data.region || "Seu estado";
